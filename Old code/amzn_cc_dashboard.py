# amzn_cc_dashboard.py (clean conId-only market data)
...
PORT     = int(os.getenv("IB_PORT", "4001"))      # LIVE Gateway
CLIENTID = int(os.getenv("IB_CLIENT_ID", "1101"))
BASE_CCY = os.getenv("BASE_CCY", "USD")
MAX_CONTRACTS = 48
STRIKE_PCT_UPPER = 0.25
FEE_PER_CONTRACT = float(os.getenv("FEE_PER_CONTRACT", "0.85"))

util.patchAsyncio()
print("=== AMZN DASH VERSION: 2025-10-14-12:15Z ===")


def _parse_expiry(e: str):
    s = str(e).strip()
    if "-" in s:
...
async def main():
    ...
    try:
        ...
        # outside RTH we’ll flip to delayed-frozen if needed
        ib.reqMarketDataType(1)  # start with REALTIME

        # Underlying on SMART with primary exchange for solid mapping
        amzn = Stock('AMZN', 'SMART', 'USD', primaryExchange='NASDAQ')
        [amzn] = await ib.qualifyContractsAsync(amzn)

        # stream spot (fallback to 1-min hist if empty)
        ...
        if not chain:
            print("No option chain metadata."); return
        ...
        if not exps:
            print("No near expiries."); return

        strikes = sorted(k for k in meta.strikes if (k >= spot and (k-spot)/spot <= STRIKE_PCT_UPPER))
        if not strikes:
            print("No OTM strikes found within range."); return

        # Build protos and resolve to real contracts (conId)
        ...
        if not real_contracts:
            print("No qualified option contracts via ContractDetails."); return

        # Subscribe: NBBO + model/Greeks on real conIds only
        for c in real_contracts:
            ib.reqMktData(c, '', snapshot=False)
            ib.reqMktData(c, '106', snapshot=False)

        # wait up to 10s; if nothing after 4s, use delayed-frozen
        for i in range(20):
            await asyncio.sleep(0.5)
            if i == 8:
                ib.reqMarketDataType(3)  # delayed-frozen fallback
