#!/usr/bin/env python3
# amzn_cc_metrics_4x_sync.py
#
# Synchronous ib_insync script:
# - Pulls AMZN option chain for the nearest 4 expiries (SMART)
# - Computes covered-call metrics + synthetic-short carry
# - Adds "All-in AR%" = AnnYield% + AnnOnSpot%
# - Keeps Δ ≤ 0.25 overwrite filter
#
# Output: one table per expiry.
...
GENERIC_TICKS = '106'    # model greeks
MAX_EXPIRIES = 4         # how many maturities to show

def safe(v):
    return None if v is None or (isinstance(v, float) and math.isnan(v)) else v

def pick_nearest_expiries(chains, n=4):
    """Return up to n soonest expiries (>= today) from SMART chain record."""
    ...
    exps = exps[:n]
    return rec, [e.strftime('%Y%m%d') for e in exps]
...
def one_expiry_block(ib: IB, spot: float, rec, expiry: str):
    """Build the metrics table for a single expiry."""
    ...
    # Build calls & puts for these strikes
    contracts = []
    ...
    for k in strikes_sel:
        ...
        if (delta is not None) and (abs(delta) <= 0.25):
            all_in = ann_yield + ann_on_spot  # requested "All-in AR"
            rows.append({
                'Expiry': expiry,
                'DTE': dte,
                'Strike': float(kf),
...
def main():
    ...
    try:
        ...
        if spot is None:
            raise RuntimeError("Failed to fetch AMZN spot. Check data permissions / duplicate client IDs.")
        ...
        rec, expiries = pick_nearest_expiries(chains, n=MAX_EXPIRIES)

        print(f"\n📈 AMZN spot: {spot:.2f} USD")
        for expiry in expiries:
            rows = one_expiry_block(ib, spot, rec, expiry)
            print(f"\n=== Expiry {expiry} | Δ≤0.25 | Strikes ~{STRIKES_AROUND*2+1} window ===")
            if rows:
                df = util.df(rows).sort_values(['All-in AR%','AnnYield%','Strike'],
                                               ascending=[False, False, True])
                # reorder columns for readability
                cols = ['Strike','Bid','Ask','Mid','Delta','IV',
                        'Premium%','AnnYield%','AnnOnSpot%','All-in AR%',
                        'ProbITM%','Breakeven','LockPnL/sh','DTE','Expiry']
                df = df[[c for c in cols if c in df.columns]]
                print(df.to_string(index=False))
            else:
                print("No strikes passed the filter (greeks/quotes may be missing). Consider raising WAIT_SEC or loosening Δ filter.")
