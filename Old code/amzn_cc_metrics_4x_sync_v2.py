#!/usr/bin/env python3
# amzn_cc_metrics_4x_sync_v2.py (hardened for delayed data & junk quotes)
...
def fetch_spot_with_fallback(ib: IB, stk: Stock, poll_live=5.0, poll_delayed=6.0):
    ...
    for _ in range(steps):
        ...
        if v is not None:
            return float(v)
    # Final fallback: recent daily close
    try:
...
def one_expiry_block(ib: IB, stk_ref: Stock, spot: float, expiry: str):
    ...
    # Use expiry as a stable key; pass dte for RV window
    ...
    for k in strikes_sel:
        ...
        prob_itm = (abs(delta) if delta is not None else (abs(delta_proxy) if delta_proxy is not None else 0.0)) * 100.0
    ...
    for r in tmp_rows:
        ...
        iv_base = r['IV'] if r['IV'] is not None else (dyn_diag['ATM_IV%']/100.0 if dyn_diag['ATM_IV%'] else 0.40)
        ...
        if rv_ann > 0:
            lo = 0.75 * rv_ann
            hi = 1.25 * rv_ann
            iv_row_eff = min(max(iv_row_eff, lo), hi)

        ST, W = _terminal_prices_logN(spot, T, dyn_drift_pct, iv_row_eff, DIST_POINTS)
        ...
        # --- Optional: sanity check WeightedAR with a small Monte Carlo
        mc_ar_val = None
        mc_ar_diff = None
        try:
            mc_ar = _mc_weighted_ar_check(
                spot, k_o, const_cash, r['DTE'], dyn_drift_pct, iv_row_eff, n_paths=2000
            )
            if mc_ar is not None and abs(mc_ar - weighted_ar) > 8.0:
                mc_ar_val = round(mc_ar, 2)
                mc_ar_diff = round(mc_ar - weighted_ar, 2)
        except Exception:
            pass
        ...
        # CVaR(5%)
        # Use weights: sort by pnl with weights
        ...
        for x, w in pairs:
            if accum + w <= cutoff:
                ...
                # partial slice of the last bin if needed
                remain = max(0.0, cutoff - accum)
                if remain > 0:
                    tail_sum += x * remain
                    tail_w += remain
                break
        cvar5 = tail_sum / max(tail_w, 1e-12)
        cvar5_ann = _ann_from_per_share(cvar5, spot, r['DTE'])

        # Stress ±15%
        ST_dn = spot * 0.85
        ST_up = spot * 1.15
        pnl_dn = const_cash - max(ST_dn - k_o, 0.0)
        pnl_up = const_cash - max(ST_up - k_o, 0.0)
        stress_dn = _ann_from_per_share(pnl_dn, spot, r['DTE'])
        stress_up = _ann_from_per_share(pnl_up, spot, r['DTE'])

        all_in_point = r['AnnYield%'] + best_ann_on_spot
        roe = (all_in_point / MARGIN_RATE) if MARGIN_RATE > 0 else None

        # Build the output row only now
        r2 = dict(r)
        r2['All-in AR%']      = round(all_in_point, 2)
        r2['AllInBreakeven']  = None if all_in_breakeven is None else round(all_in_breakeven, 2)
        ...
        r2['StressAR-15%']    = round(stress_dn, 2)
        r2['StressAR+15%']    = round(stress_up, 2)
        r2['ROE%']            = None if roe is None else round(roe, 1)

        # Attach MC results (if any)
        if mc_ar_val is not None:
            r2['_MC_AR%']  = mc_ar_val
            r2['_AR_diff%'] = mc_ar_diff

        # Heuristic anomaly flag AFTER r2 exists
        if (r2.get('ProbITM%') is not None
            and r2['ProbITM%'] > 65
            and r2.get('WeightedAR%') is not None
            and r2['WeightedAR%'] > 40):
            r2['_flag'] = 'AR>40% w/ProbITM>65%'
        ...
        for expiry in expiries:
            ...
            if rows:
                ...
                cols = [
                    ...
                    'WeightedAR%','SharpeW','Sortino','CVaR(5%)',
